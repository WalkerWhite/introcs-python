...
class _AsyncContext(object):
    ...
    def _pack(self,obj,x,y,width,height):
        import tkinter as tk
        window = tk.Toplevel(self._root)
        window.title(obj._title)
        window.protocol("WM_DELETE_WINDOW",obj.dispose)
        outer = tk.Frame(window,width=width, height=height,padx=self._PADDING,pady=self._PADDING)
        w = width -self._PADDING-self._BORDER
        h = height-self._PADDING-self._BORDER
        inner = tk.Frame(outer, width=w, height=h,borderwidth=self._BORDER,relief=tk.SUNKEN)
        canvas = tk.Canvas(inner, width=w, height=h)
        canvas.pack()
        inner.pack()
        outer.pack()
        obj._window = window
...
class _SyncContext(object):
    ...
    def __init__(self):
        ...
        This constructor will allocate the Tkinter ojects in the main thread.
        """
        import tkinter as tk
        self._root = None
        self._bkgd = None
    ...
    def refresh(self):
        ...
        This method is necessary for the synchronous context, as there is no main
        loop active.
        """
        for key in self._window:
            self._window[key]._tk_update()
